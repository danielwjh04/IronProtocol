...
- **One-tap launch from Routines.** Every routine card exposes a **Start Today's Workout** button. Tapping it activates the routine (if needed) and jumps straight to the dashboard with today's session pre-loaded — no extra nav detour after creating a routine.
...
Save — the routine becomes active and appears on the Routines list. Tap **Start Today's Workout** on the card to jump straight into today's session.
...
Either tap **Start Today's Workout** from the Routines screen, or open the home dashboard directly. Then:
