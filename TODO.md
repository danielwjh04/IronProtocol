...
- [x] Create/finalize `src/workers/embeddingWorker.ts`. Ensure it loads the `Xenova/all-MiniLM-L6-v2` model and handles the `Float32Array` conversion.
- [x] [STITCH UI] Design and implement `src/components/Search/NLPSearchBar.tsx` with a 300ms debounce.
- [x] [STITCH UI] Design and implement the dropdown renderer in the search UI. Use `cosineSimilarity` to rank the 300 exercises based on user input (e.g., "I want to grow my upper chest but I have a shoulder tweak").
- [x] [STITCH UI] Design and implement a visual "Confidence Score" (e.g., 95% Match) on the `ExerciseCard` component.
...
- [x] Execute `mergeExercises.ts` to generate the final file, then delete the `src/data/raw/` directory.
- [x] Refactor `src/utils/seedEmbeddings.ts`. The embedding generation must process the final `exercises.json` file in **chunks of 5** to prevent Out-Of-Memory (OOM) crashes in the browser worker. Add console logs for "Processed batch X of Y".
